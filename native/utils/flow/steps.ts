 ...
 *
 * Each key represents a specific navigation action or status (e.g., `left`, `right`, `u-turn`,
 * `rain`), and maps to an `InstructionStep` that defines the icon and spoken instruction for that
 * action.
 *
 * This object is used to provide user-friendly navigation instructions and status updates
 * throughout the application.
 *
 * @remarks
 * The keys are strings describing navigation steps or environmental statuses, and the values are
 * generated by the `instructionStep` function, which takes an icon name and a description.
 *
 * @example
 * ```typescript
